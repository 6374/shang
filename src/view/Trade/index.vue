<template>
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(item, index) in address" :key="index">
      ...
      <div class="bbs">
        <h5>买家留言：</h5>
        <!-- 多行文本标签 -->
        <textarea placeholder="建议留言前先与商家沟通确认" class="remarks-cont" v-model="OrderValue"></textarea>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{ userDefaultAddres.fullAddress }}</span>
        收货人：<span>{{ userDefaultAddres.consignee }} </span>
        <span> {{ userDefaultAddres.phoneNum }}</span>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    userDefaultAddres() {
      //find；查找数组中符合条件的元素返回，为最终结果，没找到返回undefiend
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let data = {
        consignee: this.userDefaultAddres.consignee,//收件人名字
        consigneeTel: this.userDefaultAddres.phoneNum,//收件人电话
        deliveryAddress: this.userDefaultAddres.fullAddress,//地址
        paymentWay: "ONLINE",
        orderComment: this.OrderValue,
