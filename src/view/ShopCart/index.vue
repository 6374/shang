      ...
      <div class="cart-body">
        ...
        <ul class="cart-list" v-for="(item, index) in cartInfoList" :key="item.id">
          <li class="cart-list-con1">
            <input type="checkbox" name="chk_list" :checked="item.isChecked == 1" @click="CheckCart(item, $event)">
          </li>
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="deleteHandler(item)">删除</a>
            <br>
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    handler: throttle(async function (type, disNum, cart) {
      ...
      // 添加物品到购物车
      await this.$store.dispatch('getShopCart', { skuId: cart.skuId, skuNum: disNum })
      this.getData()
    }, 100),
    //删除购物车商品
    async deleteHandler(cart) {
      await this.$store.dispatch('DeleCart', cart.skuId)
      this.getData()
    },
    //修改选中状态
    async CheckCart(cart, e) {
      //isChecked：string不可以是boolear
      try {
        let check = e.target.checked ? '1' : '0'
        await this.$store.dispatch('CheckCartById', { skuId: cart.skuId, isChecked: check })
        this.getData()
      } catch (error) {
        alert(error.message)
      }
